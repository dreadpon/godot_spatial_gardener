...
const PRESET_DELETE:Array = [UI_ActionThumbnail_GD.InteractionFlags.DELETE, UI_ActionThumbnail_GD.InteractionFlags.CLEAR]
const PRESET_PLANT_STATE:Array = [UI_ActionThumbnail_GD.InteractionFlags.DELETE, UI_ActionThumbnail_GD.InteractionFlags.SET_DRAG, UI_ActionThumbnail_GD.InteractionFlags.PRESS, UI_ActionThumbnail_GD.InteractionFlags.CHECK, UI_ActionThumbnail_GD.InteractionFlags.SHOW_COUNT]
...
func _generate_thumbnail():
	...
	thumb.init(element_display_size, int(float(element_display_size) * 0.32), element_interaction_flags)
...
func _queue_thumbnail(res:Resource, thumb):
	...
	if resource_path == "":
		...
		_resource_previewer.queue_resource_preview(resource_path, self, "try_assign_to_thumbnail", thumb)
...
func try_assign_to_thumbnail(path:String, preview:Texture, thumbnail_preview:Texture, thumb:Control):
	if !is_inside_tree(): return
	if preview:
		thumb.set_thumbnail(preview)
	else:
		var path_name = path.get_file()
		thumb.set_alt_text(path_name)
