...
func import_instance_transforms(file_path: String, plant_idx: int):
	...
	on_stroke_finished()
	logger.info("Successfully imported %d Placeform(s) from '%s' to index %d" % [placement_transform_dicts.size(), file_path, plant_idx])
...
func export_instance_transforms(file_path: String, plant_idx: int):
	...
	file.close()
	logger.info("Successfully exported %d Placeform(s) to '%s' at index %d" % [placement_transform_dicts.size(), file_path, plant_idx])
